...
{
    public class F03Controller
    {
        public bool ValidateF03(List<EntityF03> entity, List<Headers> headerEntity)
        {
            string sparator = "|";
            string text = "";
            ...
            try
            {
                ...
                foreach (EntityF03 item in entity)
                {
                    text =
                        item.no_rek_fasilitas + sparator +
                        item.flag_detail + sparator +
                        item.cif + sparator +
                        item.kd_jenis_surat_berharga + sparator +
                        item.sovereign_rate + sparator +
                        item.listing + sparator +
                        item.peringkat_surat + sparator +
                        item.kd_tujuan_kepemilikan + sparator +
                        item.tgl_penerbitan + sparator +
                        item.tgl_pembelian + sparator +
                        item.tgl_jatuh_tempo + sparator +
                        item.kd_valuta + sparator +
                        item.nominal + sparator +
                        item.nilai_dalam_uang_asal + sparator +
                        item.nilai_pasar + sparator +
                        item.nilai_perolehan + sparator +
                        item.prst_suku_bunga + sparator +
                        item.tunggakan + sparator +
                        item.jmlh_hari_tunggakan + sparator +
                        item.kd_kolektibilitas + sparator +
