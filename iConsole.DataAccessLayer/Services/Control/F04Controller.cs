...
namespace iConsole.DataAccessLayer.Services.Control
{
    ...
    {
        public bool ValidateF04(List<EntityF04> entity, List<Headers> headerEntity)
        {
            string sparator = "|";
            string text = "";
            ...
            try
            {
                ...
                foreach (EntityF04 item in entity)
                {
                    text =
                        item.flag_detail + sparator +
                        item.no_rekening_fasilitas + sparator +
                        item.cif + sparator +
                        item.kd_jenis_lc + sparator +
                        item.kd_tujuan_lc + sparator +
                        item.tgl_keluar + sparator +
