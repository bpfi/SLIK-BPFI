...
public partial class module_debitur_penjaminlist : BasePageList
{
    private static string TABLE_NAME = "PENJAMIN";
    private static string TABLE_NAME_HEADER = "HEADER";
    private string SEGMENT = "P01";
    private Services cv = new Services();
    private P01Controller dc = new P01Controller();

    protected void Page_Init(object sender, EventArgs e)
    {
        PAGE_LIST = "PENJAMIN";
        NEXT_PAGE = "penjaminlist.aspx";
    }
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            List<EntityP01> list = new List<EntityP01>();
            list = cv.ConvertDataTable<EntityP01>(_dt);

            bool isCreate = dc.ValidateP01(list, header);

            if (isCreate)
                Shared.ShowSuccessGritter(this, string.Format("penjaminlist.aspx"));
    ...
    protected override void SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        Response.Redirect("penjamin.aspx?action=edit&cif=" + gvwList.SelectedDataKey[0].ToString()
            + "&no_identitas_penjamin=" + gvwList.SelectedRow.Cells[2].Text
            + "&date=" + gvwList.SelectedRow.Cells[5].Text
            + "&no_rekening=" + gvwList.SelectedRow.Cells[3].Text);
    }
}
