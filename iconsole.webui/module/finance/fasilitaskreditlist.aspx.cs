...
public partial class module_debitur_fasilitaskreditlist : BasePageList
{
    private static string TABLE_NAME = "KREDIT";
    private static string TABLE_NAME_HEADER = "HEADER";
    private string SEGMENT = "F01";
    private Services cv = new Services();
    private F01Controller dc = new F01Controller();

    protected void Page_Init(object sender, EventArgs e)
    {
        PAGE_LIST = "KREDIT";
        NEXT_PAGE = "fasilitaskredit.aspx";
    }
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            List<EntityF01> list = new List<EntityF01>();
            list = cv.ConvertDataTable<EntityF01>(_dt);

            bool isCreate = dc.ValidateF01(list, header);

            if (isCreate)
                Shared.ShowSuccessGritter(this, string.Format("fasilitaskreditlist.aspx"));
    ...
    protected override void SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        Response.Redirect("fasilitaskredit.aspx?action=edit&cif=" + gvwList.SelectedDataKey[0].ToString()
             + "&no_rekening_fasilitas=" + gvwList.SelectedRow.Cells[2].Text
            + "&date=" + gvwList.SelectedRow.Cells[7].Text
            + "&no_akad_awal=" + gvwList.SelectedRow.Cells[3].Text);
    }
}